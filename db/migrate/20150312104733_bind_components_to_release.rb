  ...
  def up
    Product.all.each do |product|
      product.releases.each do |release|
        components = Component.connection.select_all("SELECT `components`.* FROM `components` INNER JOIN `components_products` ON `components`.`id` = `components_products`.`component_id` WHERE `components_products`.`product_id` = #{product.id}")
